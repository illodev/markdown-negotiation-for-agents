...
async function getPostUrl(request, slug = 'hello-markdown-world') {
  // Try by slug first
  const res = await request.get(`${WP_URL}/wp-json/wp/v2/posts?slug=${slug}&status=publish`);
  const posts = await res.json();
  if (posts.length > 0) return { id: posts[0].id, link: posts[0].link };

  // Fallback to first post
  const fallback = await request.get(`${WP_URL}/wp-json/wp/v2/posts?status=publish&per_page=1`);
...
test.describe('HTTP Content Negotiation', () => {
  ...
  test('GET single post: default Accept returns HTML', async ({ request }) => {
  ...
  test('GET single post with Accept: text/markdown returns Markdown', async ({ request }) => {
    const { link } = await getPostUrl(request);
    const res = await request.get(link, {
  ...
  test('GET single post with Accept: text/x-markdown returns Markdown', async ({ request }) => {
    const { link } = await getPostUrl(request);
    const res = await request.get(link, {
  ...
  test('Markdown response includes Vary: Accept header', async ({ request }) => {
    const { link } = await getPostUrl(request);
    const res = await request.get(link, {
      headers: { 'Accept': 'text/markdown' },
    });
  ...
  test('Markdown response includes X-Markdown-Source header', async ({ request }) => {
    const { link } = await getPostUrl(request);
    const res = await request.get(link, {
      headers: { 'Accept': 'text/markdown' },
    });
  ...
  test('Markdown response includes X-Markdown-Plugin-Version header', async ({ request }) => {
    const { link } = await getPostUrl(request);
    const res = await request.get(link, {
      headers: { 'Accept': 'text/markdown' },
    });
  ...
  test('Markdown response includes Link header for Markdown discovery', async ({ request }) => {
    const { link } = await getPostUrl(request);
    const res = await request.get(link, {
  ...
  test('HTML response includes <link rel="alternate" type="text/markdown">', async ({ request }) => {
    const { link } = await getPostUrl(request);
    const res = await request.get(link, {
      headers: { 'Accept': 'text/html' },
    });
  ...
  test('Single post ?format=markdown returns Markdown', async ({ request }) => {
    const { link } = await getPostUrl(request);
  ...
  test('REST API /jetstaa-mna/v1/markdown/:id returns markdown field', async ({ request }) => {
    const { id } = await getPostUrl(request);
    const res = await request.get(`${WP_URL}/wp-json/jetstaa-mna/v1/markdown/${id}`);
    ...
    if (res.status() === 200) {
      ...
      console.log('ℹ️ REST API endpoint requires authentication (expected)');
    }
  });

  test('REST API status endpoint works', async ({ request }) => {
    const res = await request.get(`${WP_URL}/wp-json/jetstaa-mna/v1/status`);
    expect([200, 401]).toContain(res.status());
    if (res.status() === 200) {
      const body = await res.json();
      expect(body).toHaveProperty('enabled');
      console.log('✅ REST API /jetstaa-mna/v1/status works:', JSON.stringify(body));
    } else {
  ...
  test('WP REST API posts include markdown field', async ({ request }) => {
    ...
    const body = await res.json();
    expect(body).toHaveProperty('markdown');
    console.log('✅ REST API posts have markdown field, type:', typeof body.markdown);
  });
  ...
  test('Hello Markdown World post converts correctly', async ({ request }) => {
