...
test.describe("WordPress Installation & Plugin Setup", () => {
	test("install WordPress", async ({ page }) => {
		...
		const searchCheckbox = page.getByRole("checkbox", {
			name: /discourage search engines/i,
		});
		if (
			await searchCheckbox.isVisible({ timeout: 2000 }).catch(() => false)
		) {
			if (!(await searchCheckbox.isChecked()))
				await searchCheckbox.check();
		}

		const weakPassCheckbox = page.getByRole("checkbox", {
			name: /confirm use of weak password/i,
		});
		if (
			await weakPassCheckbox
	...
	test("configure permalinks and activate plugin", async ({ page }) => {
		...
		await page.waitForSelector(
			'tr[data-slug="markdown-negotiation-for-agents"]',
			{ timeout: 10000 },
		);
		...
		if (
			await activateLink.isVisible({ timeout: 3000 }).catch(() => false)
		) {
	...
	test("create a sample post for testing", async ({ page }) => {
		...
		if (posts.length > 0) {
			console.log(
				`ℹ️ Sample post already exists: ID ${posts[0].id} — ${posts[0].link}`,
			);
			return;
		}
		...
		const response = await page.request.post(
			`${WP_URL}/wp-json/wp/v2/posts`,
			{
				...
				data: {
					...
					content:
						'<h2>Introduction</h2><p>This is a <strong>test post</strong> to verify Markdown negotiation.</p><ul><li>Item one</li><li>Item two</li><li>Item three</li></ul><h2>Code Example</h2><pre><code>function hello() {\n  return "world";\n}</code></pre>',
					status: "publish",
				},
			},
		);
